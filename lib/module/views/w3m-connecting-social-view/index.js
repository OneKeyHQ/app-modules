...
import { ConnectionController, ConnectorController, EventsController, ModalController, RouterController, SnackController, WebviewController } from '@reown/appkit-core-react-native';
...
export function ConnectingSocialView() {
  ...
  const {
    processingAuth
  } = useSnapshot(WebviewController.state);
  const {
  ...
  const onConnect = useCallback(async () => {
    try {
      if (!WebviewController.state.connecting && provider && ConnectionController.state.selectedSocialProvider) {
        const {
          uri
  ...
  const socialMessageHandler = useCallback(async url => {
    try {
      if (url.includes('/sdk/oauth') && ConnectionController.state.selectedSocialProvider && authConnector && !WebviewController.state.processingAuth) {
        ...
        ConnectionController.setConnectedSocialProvider(ConnectionController.state.selectedSocialProvider);
        WebviewController.setConnecting(false);
        EventsController.sendEvent({
          type: 'track',
  ...
  }, processingAuth ? 'Loading user data' : `Continue with ${StringUtil.capitalize(selectedSocialProvider)}`), /*#__PURE__*/React.createElement(Text, {
    variant: "small-400",
    color: "fg-200"
