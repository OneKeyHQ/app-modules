...
| **Firmware artifacts (native)** | Runtime-certificate HTTPS fault server, exact 4 MiB / 83,854,948-byte streaming profiles, process/runtime memory probes, ZIP portability fixtures, and three-trial device drivers | this module: [`conformance/firmware/`](./firmware/) | `./conformance/firmware/run-ios.sh build-only` or `./conformance/firmware/run-android.sh build-only`; omit `build-only` only with the matching device driver configured |
