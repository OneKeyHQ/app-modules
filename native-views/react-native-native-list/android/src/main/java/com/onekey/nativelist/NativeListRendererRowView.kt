...
internal abstract class NativeListRendererRowView(protected val reactContext: ThemedReactContext) :
    ...
    init {
        ...
        setOnClickListener {
            current
                ...
                ?.let {
                    onRowPress?.invoke(it, NativeListActionOrigin(this, this, bindingEpoch, "row"))
                }
        }
            ...
            item.styledHeight?.let(::stylePx)
                ?: if (item.json.has("height"))
                    when (item.json.optString("heightRounding")) {
                        "floor" ->
                            (item.json.optDouble("height") * resources.displayMetrics.density)
                                .toInt()
    ...
    protected fun emitAction(
        ...
        if (key.isNotEmpty() && !item.json.optBoolean("disabled"))
            onAction?.invoke(
                ...
                key,
                target,
                NativeListActionOrigin(view, this, bindingEpoch, source, slot),
            )
    }
    ...
    private fun appearance() {
        ...
        val showSelected =
            showsSelection &&
                selected &&
                (listLayout != "sectioned" || item.json.optBoolean("selected"))
        val backgroundName =
        ...
        val radius =
            if (container.has("cornerRadius"))
                stylePx(container.optDouble("cornerRadius")).toFloat()
