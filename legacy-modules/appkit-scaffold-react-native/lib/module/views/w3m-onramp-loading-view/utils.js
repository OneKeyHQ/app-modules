...
export function parseOnRampRedirectUrl(url) {
  try {
    ...
    const purchaseAmount = purchaseAmountParam ? (() => {
      ...
      return isNaN(parsed) ? OnRampController.state.selectedQuote?.destinationAmount ?? null : parsed;
...
export function createEmptyOnRampResult() {
  return {
