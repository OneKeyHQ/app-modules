...
import { AccountController, EventsController, NetworkController, RouterController, SwapController } from '@reown/appkit-core-react-native';
...
export function SwapView() {
  ...
  const showSwitch = myTokensWithBalance && myTokensWithBalance.findIndex(token => token.address === SwapController.state.toToken?.address) >= 0;
  ...
  const getActionButtonState = () => {
    ...
    if (SwapController.state.inputError) {
      return {
        text: SwapController.state.inputError,
        disabled: true
      };
    }
    return {
  ...
  const onSourceMaxPress = () => {
    const isNetworkToken = SwapController.state.sourceToken?.address === NetworkController.getActiveNetworkTokenAddress();
    ...
    if (_balance) {
      ...
      const amountOfTokenGasRequires = NumberUtil.bigNumber(_gasPriceInUSD.toFixed(5)).dividedBy(_sourceTokenPriceInUSD);
      const maxValue = isNetworkToken ? NumberUtil.bigNumber(_balance).minus(amountOfTokenGasRequires) : NumberUtil.bigNumber(_balance);
  ...
  return /*#__PURE__*/React.createElement(ScrollView, {
    ...
    editable: false
  }), showSwitch && /*#__PURE__*/React.createElement(IconLink, {
  ...
  })), showDetails && /*#__PURE__*/React.createElement(SwapDetails, null), /*#__PURE__*/React.createElement(Button, {
