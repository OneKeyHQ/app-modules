...
### Features
- **background-thread**: New `restart(mode, reason)` TurboModule method intended to replace direct use of `react-native-restart`. `mode` is `'ui'` (reload the main JS runtime only; bg stays hot — for language/currency/devSettings) or `'all'` (reload both runtimes — for OTA install/switch, resetData, where the JS bundle on disk has changed and the two runtimes cannot keep running mismatched moduleId tables). The spec exports a `RestartMode = 'ui' | 'all'` string-literal union (deliberately not a TS `enum` — `enum` produces runtime JS, isn't on the TurboModule codegen's supported-type whitelist, and forces consumers to import a runtime value alongside the type; the literal union erases to nothing and gives the same compile-time safety). `reason` is forwarded to `RCTTriggerReloadCommandListeners` and host logs so production restarts are attributable.
- **background-thread (iOS)**: `BackgroundThreadManager.restartWithMode:reason:completion:` sequences SharedRPC quiesce → optional bg host release (`mode=all`) → `RCTTriggerReloadCommandListeners`, all on the main thread. After main reload, AppDelegate.hostDidStart re-arms the "main" SharedRPC listener; for `mode=all` it also re-spawns the bg host because `isStarted` was reset. A two-stage post-reload health-check (`dispatch_after` on the main queue, scheduled before the reload triggers) self-heals integration omissions: stage 1 at ~3s either declares success (both `mainSharedBridgeInstalled` and `isStarted` are healthy) or reschedules stage 2 — earlier designs short-circuited "main ready, bg not ready" at stage 1, but hosts that gate `startBackgroundRunner` on async work can race that path and have their real start call silently dropped, so stage 1 now always defers self-respawn to stage 2. Stage 2 at ~6s total is the final verdict, with `[BTLogger error:]` for any remaining main-side gap and self-respawn for the bg-side. Self-respawn replays the host's last entry URL — cached in `startBackgroundRunnerWithEntryURL:` via a new `lastEntryURL` property — instead of falling back to the bundled `background.bundle`: critical on OTA-equipped hosts where loading the IPA's bundled bg bundle while main runs the OTA-updated main bundle would moduleId-mismatch and crash on the next cross-runtime RPC. The bundled default name `"background.bundle"` is treated as "no real cache" in the self-respawn path so the OTA-mismatch warning still fires for hosts that bootstrapped with the default and never swapped to an OTA-resolved URL. `startBackgroundRunnerWithEntryURL:`'s early-return path now warn-logs when called with a different URL than the active one (the self-respawn-vs-host-async-start race signal) so production traces can distinguish "bg is on the wrong URL" from a generic startup bug. Concurrent `restart()` calls are made safe via a monotonic `restartGeneration` counter: each invocation captures its own generation and each health-check stage bails if a newer restart has superseded it. The implementation deliberately avoids `RCTJavaScriptDidLoadNotification` because that notification's post timing is unreliable in bridgeless / NewArch, and an observer that never fires would leak across reloads. `isStarted` and `lastEntryURL` are both `atomic` to give cross-thread reads (post-reload check on main; writers on any thread per the public API) a proper memory barrier. Header doc on `BackgroundThreadManager.h` makes the full post-reload contract explicit.
