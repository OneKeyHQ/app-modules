...
- **background-thread (iOS)**: `BackgroundThreadManager.restartWithMode:reason:completion:` sequences SharedRPC quiesce → optional bg host release (`mode=all`) → `RCTTriggerReloadCommandListeners`, all on the main thread. After main reload, AppDelegate.hostDidStart re-arms the "main" SharedRPC listener; for `mode=all` it also re-spawns the bg host because `isStarted` was reset. A delayed (`dispatch_after`, `kRestartHealthCheckDelaySeconds = 3.0`) post-reload health-check self-respawns the bg runtime if the host AppDelegate failed to call `startBackgroundRunner` (idempotent via the existing `isStarted` guard, so a correctly-wired host pays nothing — but the self-respawn loses any custom entry URL set via `startBackgroundRunnerWithEntryURL:` because `reactNativeFactoryDelegate` was released; a `[BTLogger warn:]` calls this out so OTA paths aren't silently lost), and logs a loud `[BTLogger error:]` if the host failed to call `installSharedBridgeInMainRuntime:` on the new RCTHost — otherwise that integration bug surfaces only as silent main→bg RPC drop in production. Concurrent `restart()` calls are made safe via a monotonic `restartGeneration` counter: each invocation captures its own generation and the health-check bails if a newer restart has superseded it. The implementation deliberately avoids `RCTJavaScriptDidLoadNotification` because that notification's post timing is unreliable in bridgeless / NewArch, and an observer that never fires would leak across reloads. Header doc on `BackgroundThreadManager.h` makes the full post-reload contract explicit.
