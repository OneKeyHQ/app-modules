...
- **background-thread (iOS)**: `BackgroundThreadManager.restartWithMode:reason:completion:` sequences SharedRPC quiesce → optional bg host release (`mode=all`) → `RCTTriggerReloadCommandListeners`, all on the main thread. After main reload, AppDelegate.hostDidStart re-arms the "main" SharedRPC listener; for `mode=all` it also re-spawns the bg host because `isStarted` was reset. A one-shot `RCTJavaScriptDidLoadNotification` observer self-respawns the bg runtime ~1.5s after the new main bridge loads if the host AppDelegate failed to call `startBackgroundRunner` (idempotent via the existing `isStarted` guard, so a correctly-wired host pays nothing), and logs a loud `[BTLogger error:]` if the host failed to call `installSharedBridgeInMainRuntime:` on the new RCTHost — otherwise that integration bug surfaces only as silent main→bg RPC drop in production. Header doc on `BackgroundThreadManager.h` makes the post-reload contract explicit.
